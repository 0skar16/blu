...
fn to_lua(statement: Statement, ind: u8, do_ind: bool) -> String {
    ...
    match statement {
        Statement::Call(called, args, standalone) => {
            ...
            buf.push('(');
            let s_args: Vec<String> = args.into_iter().map(|arg| to_lua(arg, ind, false)).collect();
        ...
        Statement::Operation(operand, operation, operator) => {
            ...
            buf.push_str(&format!("{} ", match operation {
                Operation::Add => "+",
                Operation::Sub => "-",
                Operation::Div => "/",
                Operation::Mul => "*",
        ...
        Statement::For(iter, block) => {
            ...
            match iter {
                BluIterator::Numerical(iterative, init, end, step) => {
                    buf.push_str(&format!("{} = {}, {}", iterative, to_lua(*init, ind, false), to_lua(*end, ind, false)));
                    if let Some(step) = step {
                        buf.push_str(&format!(", {}", to_lua(*step, ind, false)));
                    }
                },
                BluIterator::Each(iterative, iterator) => {
                    buf.push_str(&format!("{} in pairs({})", iterative.join(", "), to_lua(*iterator, ind+1, false)));
                },
                BluIterator::Iterator(iterative, iterator) => {
                    buf.push_str(&format!("{} in {}", iterative.join(", "), to_lua(*iterator, ind+1, false)));
                },
            }
            buf.push_str(" do\n");
            for stmt in block.0 {
